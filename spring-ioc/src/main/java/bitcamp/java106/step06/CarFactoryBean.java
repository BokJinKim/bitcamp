...
public class CarFactoryBean implements FactoryBean<Car> {
    ...
    public void setModel(String model) {
        System.out.println("CarFactoryBean().setModel() 생성자 호출됨.");
        this.model = model;
    }
    
    @Override
    public Car getObject() throws Exception {
        System.out.println("CarFactoryBean().getObject() 생성자 호출됨.");
        // 객체를 생성해서 리턴하는 메서드이다.
    ...
    public Class<?> getObjectType() {
        System.out.println("CarFactoryBean().getObjectType() 생성자 호출됨.");
        return Car.class;
    }
}
